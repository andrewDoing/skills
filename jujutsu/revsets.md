...
```bash
# Your commits not yet on main (use --no-pager for agents)
...
jj --no-pager log -r "file('src/main.rs')"

# Commits by you
jj --no-pager log -r "author('your-email')"

# Commits with 'fix' in message
jj --no-pager log -r "description('fix')"

# Commits with conflicts
jj --no-pager log -r "conflict()"
```
...
jj --no-pager log -r "main..feature/x"
```
...
```bash
# Log with revset (use --no-pager for agents)
jj --no-pager log -r "trunk()..@ | @+"

# Diff range
jj --no-pager diff --from main --to @
...
1. Use `jj --no-pager log -r "all()"` sparingly - can be huge
2. Prefer change IDs over commit IDs for refs that might change
3. `trunk()` auto-detects main/master/trunk
4. Combine with `-p` to see patches: `jj --no-pager log -r "file('x')" -p`
5. Test revsets with `jj --no-pager log -r "<revset>"` before using in other commands
6. **For agents:** Always use `--no-pager` to prevent blocking on pager
