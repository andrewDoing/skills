...
> **Agent/Script Usage:** When running jj in non-interactive contexts, use `jj --no-pager <command>` for commands that display output (log, diff, show, op log, evolog). Always use `-m "message"` with commit/describe/new to avoid opening an editor.
...
```bash
# Find the commit you want to edit (use --no-pager for agents)
jj --no-pager log
...
# Interactive squash (select what to move) - AVOID in agent contexts
# jj squash -i
```

### Splitting a Commit

**Note:** `jj split` is interactive and cannot be used in agent/script contexts.

```bash
# Split current commit (INTERACTIVE - opens editor)
jj split
...
```bash
# 1. Ensure your changes are committed (use --no-pager for agents)
jj --no-pager log  # Review your commits
...
# Resolve interactively (INTERACTIVE - not for agents)
# jj resolve

# For agents: use non-interactive resolution
jj resolve --tool=:ours    # Accept our changes
jj resolve --tool=:theirs  # Accept their changes

# Or resolve specific file non-interactively
jj resolve --tool=:ours src/main.rs
```
...
# See operation history (use --no-pager for agents)
jj --no-pager op log
